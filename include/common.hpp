...
void nms(std::vector<Yolo::Detection> &res,
  ...
  for (auto &it: m) {
    ...
    for (size_t m = 0; m < dets.size(); ++m) {
      auto &item = dets[m];
      if (classes.empty() or std::find(classes.begin(), classes.end(), item.class_id) != classes.end()) {
        res.push_back(item);
      }
...
ILayer *focus(INetworkDefinition *network,
              ...
              std::string lname) {
  ISliceLayer
      *s1 = network->addSlice(input, Dims3{0, 0, 0}, Dims3{inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2}, Dims3{1, 2, 2});
  ISliceLayer
      *s2 = network->addSlice(input, Dims3{0, 1, 0}, Dims3{inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2}, Dims3{1, 2, 2});
  ISliceLayer
      *s3 = network->addSlice(input, Dims3{0, 0, 1}, Dims3{inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2}, Dims3{1, 2, 2});
  ISliceLayer
      *s4 = network->addSlice(input, Dims3{0, 1, 1}, Dims3{inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2}, Dims3{1, 2, 2});
